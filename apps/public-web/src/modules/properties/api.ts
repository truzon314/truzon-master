...
export async function getSharedMapDetails(
  token: string,
  password?: string
): Promise<{ project: any; layers: CmsMapLayer[]; shareLink: any } | null> {
  const headers: Record<string, string> = {};
  if (password) headers["x-share-password"] = password;
  const { data } = await cmsFetch<{ project: any; layers: CmsMapLayer[]; shareLink: any }>(
    `/public/mapping/${token}`,
    { headers }
  );
  return data;
}
