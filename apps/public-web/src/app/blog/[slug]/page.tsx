...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post && !staticPost) return { title: "Article Not Found" };
  return buildMetadata({
    seo: post?.seo,
    path: `/blog/${slug}`,
    fallbackTitle: (post?.title || staticPost?.title) ?? "",
    fallbackDescription: post?.excerpt || staticPost?.excerpt || "",
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = cmsPost ? toBlogPost(cmsPost) : staticPost!;

  return (
    <>
      {cmsPost?.seo?.schema_jsonld ? (
        <JsonLd data={cmsPost.seo.schema_jsonld} />
      ) : cmsPost ? (
        <JsonLd data={articleJsonLd(cmsPost, SITE_URL)} />
      ) : null}
      <PageHero
        title={post.title}
